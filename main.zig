...
fn eval_single_board_position(board: Board_s, model: *nn.Network(nn_type)) !void {
...
fn play_engine_game(engine: *nn.Network(nn_type)) !void {
    ...
    while (board.check_win() == 0 and num_move < 300) {
        ...
        for (pos_moves.items) |mv| {
            ...
            try eval_single_board_position(move_to_eval, engine);
        }
    ...
    try eval_single_board_position(board, engine);
}
